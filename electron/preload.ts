...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  modsModrinthSearch: (query: string, contentType?: ModContentType, gameVersion?: string, sortBy?: ModSort, page?: number) => ipcRenderer.invoke('mods:modrinth-search', query, contentType, gameVersion, sortBy, page) as Promise<ModSearchResponse>,
  ...
  modsCurseforgeSearch: (query: string, contentType?: ModContentType, gameVersion?: string, modLoader?: string, sortBy?: ModSort, page?: number) => ipcRenderer.invoke('mods:curseforge-search', query, contentType, gameVersion, modLoader, sortBy, page) as Promise<ModSearchResponse>,
  ...
  modsCurseforgeCategories: () => ipcRenderer.invoke('mods:curseforge-categories') as Promise<Array<{ id: number; slug: string; name: string }>>,
  ...
  getForgeVersions: (mcVersion: string) => ipcRenderer.invoke('minecraft:get-forge-versions', mcVersion) as Promise<string[]>,
  ...
  getNeoForgeVersions: (mcVersion: string) => ipcRenderer.invoke('minecraft:get-neoforge-versions', mcVersion) as Promise<string[]>,
  ...
  importModrinthModpack: (buildName: string, projectSlug: string, versionId?: string) => ipcRenderer.invoke('build:import-modrinth', buildName, projectSlug, versionId) as Promise<{ success: boolean; error?: string; version?: string; modLoader?: string }>,
  importCurseforgeModpack: (buildName: string, modId: number, fileId: number) => ipcRenderer.invoke('build:import-curseforge', buildName, modId, fileId) as Promise<{ success: boolean; error?: string; version?: string; modLoader?: string }>,
  openAndImportModpack: () => ipcRenderer.invoke('build:open-and-import') as Promise<{ success: boolean; error?: string; name?: string; description?: string; icon?: string; version?: string; modLoader?: string; source?: 'modrinth' | 'curseforge'; intentPath?: string }>,
  onImportProgress: (callback: (progress: { current: number; total: number; message: string }) => void) => subscribe('import:progress', callback),
  ...
  cloudRegister: (username: string, password: string) => ipcRenderer.invoke('cloud:register', username, password) as Promise<{ success: boolean; error?: string }>,
  ...
  cloudGetFiles: (token: string, category?: string) => ipcRenderer.invoke('cloud:get-files', token, category) as Promise<{ success: boolean; files?: any[]; error?: string }>,
