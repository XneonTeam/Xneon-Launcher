...
function getModrinthAppProfilesDir(): string {
  const home = app.getPath("home")
  const isWindows = process.platform === "win32"
  const appData = isWindows ? (process.env.APPDATA || "C:\\Windows\\System32\\config\\systemprofile\\AppData\\Roaming") : ""
...
function getModrinthAppDbPath(): string {
  const home = app.getPath("home")
  const isWindows = process.platform === "win32"
  const appData = isWindows ? (process.env.APPDATA || "C:\\Windows\\System32\\config\\systemprofile\\AppData\\Roaming") : ""
...
async function discoverModrinthAppInstances(): Promise<LauncherInstance[]> {
  const profilesDir = getModrinthAppProfilesDir()
  const dbPath = getModrinthAppDbPath()
...
async function discoverAllInstances(): Promise<LauncherInstance[]> {
