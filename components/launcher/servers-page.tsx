...
import { IconRefresh, IconPlus, IconSearch, IconServer, IconStar, IconTrash, IconX, IconPlugConnected, IconCircleX } from "@tabler/icons-react"
...
async function fetchServerStatus(ip: string): Promise<ServerStatus> {
  const res = await fetch(`https://mcskinapi-three.vercel.app/status/${ip}`)
...
export function ServersPage() {
  ...
  // Save servers + write servers.dat whenever servers change
  useEffect(() => {
  ...
  const renderServer = (server: ServerEntry) => {
            ...
            <>
              ...
              <div className="flex items-center gap-4 mt-1">
                <span className="text-xs text-muted-foreground">
                  ...
                  {formatPlayers(status?.players_max ?? 0)} игроков
                </span>
            ...
            <>
              {favoriteServers.length > 0 && (
                <div>
                  ...
                  <div className="space-y-2 mt-2">{favoriteServers.map(renderServer)}</div>
                </div>
              )}
              {onlineServers.filter((s) => s.loading || s.error || s.status?.online).length > 0 && (
                <div>
                  ...
                  <div className="space-y-2 mt-2">{onlineServers.map(renderServer)}</div>
                </div>
              )}
              {offlineServers.length > 0 && (
                <div>
                  ...
                  <div className="space-y-2 mt-2">{offlineServers.map(renderServer)}</div>
                </div>
              )}
            </>
          )}
